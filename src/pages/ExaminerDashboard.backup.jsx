...
// ── Results Tab (sorted by exam) ─────────────────────────────────────────
const ResultsTab = ({ user, results, exams }) => {
...
const HelpTab = () => {
  ...
  const handleTicketSubmit = async (e) => {
    ...
    try {
      ...
      await api.post('/tickets', { ...ticketForm, studentId: user.id });
      setTicketMsg('Ticket submitted successfully! We\'ll respond soon.');
  ...
  const faqs = [
    ...
    },
    {
      ...
      a: 'When creating/editing an exam, click "AI Generate". You can generate questions by topic (e.g., "Binary Trees") or paste syllabus/document text. Select question type (MCQ, Multi-Correct, Short Answer, Coding), count, and difficulty. The AI will generate questions that you can review and edit before adding to your exam.'
    },
    {
    ...
    {
      q: 'How do I grade subjective questions?',
      a: 'In the Exams tab, click "Results" for any exam. For submissions with short answer or coding questions, click the "Grade" button. Enter marks for each subjective question (up to the maximum marks), then click "Save Grades". The total score will be automatically updated.'
    },
    {
    ...
    },
    {
      ...
      a: 'Four types: (1) MCQ - single correct answer, (2) Multiple Correct - multiple correct answers, (3) Short Answer - text response requiring manual grading, (4) Coding - code response requiring manual grading. You can enable negative marking for MCQ and Multiple Correct questions.'
    },
    {
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {[
          ...
          { icon: PenLine, title: 'Manual Grading', desc: 'Grade short answer and coding questions manually', color: 'text-orange-600', bg: 'bg-orange-50' },
      ...
      <div className="bg-gray-900 text-white rounded-2xl p-6">
        <h2 className="font-bold text-lg mb-2">Need More Help?</h2>
        <p className="text-gray-300 text-sm mb-4">Can't find what you're looking for? Submit a support ticket and our team will help you.</p>
        <button onClick={() => setShowTicket(true)} className="px-4 py-2 bg-white text-gray-900 rounded-xl text-sm font-bold hover:bg-gray-100 transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <form onSubmit={handleTicketSubmit} className="p-6 space-y-4">
              {ticketMsg && (
                <div className={`px-4 py-3 rounded-xl text-sm font-medium ${ticketMsg.includes('success') ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-600 border border-red-200'}`}>{ticketMsg}</div>
              )}
              <div>
...
const ExamResultRow = ({ r, exam, onGraded }) => {
  ...
  // Find subjective questions (short_answer / coding) that need manual grading
  const subjectiveQs = exam?.questions?.map((q, i) => ({ ...q, idx: i }))
    .filter(q => q.type === 'short_answer' || q.type === 'coding') || [];
              ...
              <div className="divide-y divide-gray-100 max-h-64 overflow-y-auto">
                {exam?.questions?.map((q, i) => {
                  ...
                  const isCorrect = q.type === 'mcq' ? ans === q.correctOptionIndex :
                    q.type === 'multiple_correct' ? JSON.stringify([...(q.correctOptionIndices||[])].sort()) === JSON.stringify([...(Array.isArray(ans)?ans:[])].sort()) : null;
                  return (
                    <div key={i} className="px-4 py-3">
                      <p className="text-xs font-semibold text-gray-700 mb-1">Q{i+1}: {q.questionText}</p>
                      {(q.type === 'mcq' || q.type === 'multiple_correct') && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-xs font-bold px-2 py-0.5 rounded ${isCorrect ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                            {q.type === 'mcq' ? (ans !== undefined && ans !== null ? q.options?.[ans] || `Option ${ans}` : 'Not answered') : (Array.isArray(ans) && ans.length ? ans.map(a => q.options?.[a]).join(', ') : 'Not answered')}
                          </span>
                          {isCorrect !== null && <span className={`text-xs ${isCorrect ? 'text-green-600' : 'text-red-500'}`}>{isCorrect ? '✓ Correct' : '✗ Wrong'}</span>}
                        </div>
                      )}
          ...
          <td colSpan={6} className="px-4 pb-4 bg-orange-50">
            <div className="rounded-xl border border-orange-200 overflow-hidden">
              <div className="px-4 py-2 bg-orange-100 text-xs font-bold text-orange-700 uppercase tracking-wider flex items-center gap-2">
                <PenLine className="h-3.5 w-3.5" /> Manual Grading — Subjective Questions
              </div>
              <div className="divide-y divide-orange-100">
                {subjectiveQs.map(q => (
                  <div key={q.idx} className="px-4 py-3 flex items-start gap-4">
                    <div className="flex-1">
                      <p className="text-xs font-semibold text-gray-700">Q{q.idx+1}: {q.questionText}</p>
                      <p className="text-xs text-gray-500 mt-1">Student: <span className="text-gray-800">{r.answers?.[q.idx] || 'Not answered'}</span></p>
                  ...
                  {questions.map((q, qi) => (
                    <div key={qi} className="p-4 border border-gray-200 rounded-xl bg-gray-50/50 relative group">
                      ...
                      <div className="flex items-center gap-2 mb-3 flex-wrap">
                        ...
                        {['mcq','multiple_correct','short_answer','coding'].map(t => (
                          <button key={t} type="button" onClick={() => updateQ(qi, 'type', t)}
                            ...
                            {t === 'mcq' ? 'MCQ' : t === 'multiple_correct' ? 'Multi-Correct' : t === 'short_answer' ? 'Short Answer' : 'Coding'}
                          </button>
                        ))}
                      ...
                      {/* Short answer / coding model answer */}
                      {(q.type === 'short_answer' || q.type === 'coding') && (
                        <div>
                          <label className="text-xs font-bold text-gray-500 mb-1 block">{q.type === 'coding' ? 'Model Solution' : 'Model Answer'} <span className="text-gray-400 font-normal">(for reference)</span></label>
                          <textarea rows={q.type === 'coding' ? 4 : 2} value={q.modelAnswer} onChange={e => updateQ(qi, 'modelAnswer', e.target.value)}
                            className={`w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500 resize-y ${q.type === 'coding' ? 'font-mono text-xs' : ''}`}
                            placeholder={q.type === 'coding' ? 'def solution(n):\n    # write your solution here' : 'Expected answer...'} />
                        </div>
                      )}
                    </div>
                  ))}
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8">
          <div className="max-w-6xl mx-auto">
            ...
            {activeTab === 'results' && <ResultsTab user={user} results={results} exams={exams} />}
